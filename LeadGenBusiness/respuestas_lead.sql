...
SELECT client_id, count(site_id), monthname(created_datetime),year(created_datetime) FROM sites 
WHERE client_id=1 
group by site_id;
...
Select s.domain_name, count(s.client_id) from leads as l, sites as s, clients as c
where c.client_id = s.client_id 
and l.site_id = s.site_id
and registered_datetime >= '2011/01/01' AND registered_datetime < '2011/02/15'
group by l.site_id;
...
Select c.first_name, sum(s.client_id) from leads as l, sites as s, clients as c
where c.client_id = s.client_id 
and l.site_id = s.site_id
and registered_datetime >= '2011/01/01' AND registered_datetime < '2011/12/31'
group by l.site_id;
